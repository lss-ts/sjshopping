...
<script>
    ...
    app.controller('mycontroller',function ($scope,$http) {
        ...
        //   console.log($scope.userinfo);
            ...
            console.log(str);
            // 向数据库发送数据
            $http.post('__APP__/Shop/admin',str,{headers:{"content-type":"application/x-www-form-urlencoded"}}).success(function (res) {
                console.log(res);
                if(res.status==1){
                    alert('登录成功！');
                    $scope.reset();
                    setTimeout("location.href='__APP__/Shop/'",100);
                }else{
                    alert('用户名或密码错误!!');
                     $scope.reset();
                }
